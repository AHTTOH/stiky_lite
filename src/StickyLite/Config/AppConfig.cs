...
namespace StickyLite.Config
{
    ...
    public class AppConfig
    {
        ...
        public bool IsCustomColor { get; set; } = false; // 커스텀 색상 사용 여부
        public string CustomColor { get; set; } = "#FFFFE0"; // 커스텀 색상
        ...
        public Color GetThemeBackgroundColor()
        {
            try
            {
                ...
                else if (ColorTheme >= 0 && ColorTheme < PastelColors.Length)
                {
        ...
        {
            CustomColor = ColorTranslator.ToHtml(color);
